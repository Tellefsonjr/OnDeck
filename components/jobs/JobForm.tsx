...
const JobForm = (props) => {
        ...
        return (
            daysOfWeek.map((day) => {
                ...
                return (
                    <Chip
                        style={{
                            backgroundColor: selected ? Colors.primaryLight : Colors.light.background,
                        ...
                        onPress={() => {
                            setFieldValue('dates.days', selected ? _.without(days, day) : _.concat(days, day))
                        }}
                    >
                        {day}
                    </Chip>
    ...
    const handleConfirm = (date, setFieldValue, values) => {
        if (dateType.type == 'date') {
            setFieldValue(dateType.value == 'start' ? 'dates.date_start' : 'dates.date_end', date);
        } else {
            setFieldValue(dateType.value == 'start' ? 'dates.time_start' : 'dates.time_end', date);
        }
        hideDatePicker();
    };
    const displayTime = (time, locale) => {
        let formattedTime = time.toLocaleTimeString(locale).match(/^([0-9]{1,2}:[0-9]{2}).*( AM| PM)$/);
                    ...
                    <View style={styles.formContainer}>
                        ...
                        >
                            ...
                            <View>
                                ...
                                <View style={[{ alignItems: 'center', justifyContent: 'space-evenly', marginTop: 5, }]}>
                                    <Button
                                        style={[styles.submitButton, { width: '100%', marginVertical: 5, }]}
                                        icon="calendar"
                                        mode="contained"
                                        onPress={() => showDatePicker({ value: 'start', type: 'date' })}
                                        color={isValid ? Colors.primary : Colors.primary.light}
                                    >
                                        {values.dates.date_start ? values.dates.date_start.toString().substring(0, 15) : 'Start Date'}
                                    </Button>
                                    <Button
                                        style={[styles.submitButton, { width: '100%', marginVertical: 5, }]}
                                        icon="calendar"
                                        mode="contained"
                                        onPress={() => showDatePicker({ value: 'end', type: 'date' })}
                                        color={isValid ? Colors.primary : Colors.primary.light}
                                    >
                                        {values.dates.date_end ? values.dates.date_end.toString().substring(0, 15) : 'End Date'}
                                    </Button>
                                    <View style={[styles.horizontal, { width: '100%', marginVertical: 5, justifyContent: 'space-evenly' }]}>
                                        <Button
                                            style={[styles.submitButton]}
                                            icon="calendar-clock"
                                            mode="contained"
                                            onPress={() => showDatePicker({ value: 'start', type: 'time' })}
                                            color={isValid ? Colors.primary : Colors.primary.light}
                                        >
                                            {values.dates.time_start ? displayTime(values.dates.time_start, 'en-us') : 'Time Start'}
                                        </Button>
                                        <Button
                                            style={[styles.submitButton]}
                                            icon="calendar-clock"
                                            mode="contained"
                                            onPress={() => showDatePicker({ value: 'end', type: 'time' })}
                                            color={isValid ? Colors.primary : Colors.primary.light}
                                        >
                                            {values.dates.time_end ? displayTime(values.dates.time_end, 'en-us') : 'Time End'}
                                        </Button>
                                    </View>
                                </View>
                            ...
                            <View>
                                <View style={[styles.horizontal, { justifyContent: 'space-evenly'}]}>
